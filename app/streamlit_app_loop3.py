...
st.set_page_config(page_title="Loop 3 Counterfactual Example Selector", layout="wide")
...
CONTROL_TYPE_STYLE = {
    'lexical-difficulty-substitution': {'color': '#e57373', 'num': 1},
    'lexical-idiom':                  {'color': '#64b5f6', 'num': 2},
    'restructure-passive':            {'color': '#81c784', 'num': 3},
    'restructure-participle':         {'color': '#4db6ac', 'num': 4},
    'restructure-coordination':       {'color': '#ffb74d', 'num': 5},
    'insert-clause':                  {'color': '#ba68c8', 'num': 6},
    'delete-modifier':                {'color': '#a1887f', 'num': 7},
    'negation-complexity':            {'color': '#ffd54f', 'num': 8},
}
...
# app 디렉토리 바로 아래에 있는 파일 사용
current_dir = os.path.dirname(os.path.abspath(__file__))
loop3_data_path = os.path.join(current_dir, 'loop_3_converted.json')

with open(loop3_data_path, 'r', encoding='utf-8') as f:
    loop3_data = json.load(f)

print(f"데이터 로드 성공: {loop3_data_path}")
print(f"총 loop3 데이터: {len(loop3_data)}개")

# 5개 버전으로 분할 (각 버전: 36개씩)
version = st.sidebar.selectbox("Select survey version", [1, 2, 3, 4, 5])

# loop3_data를 5개 버전으로 분할 (각 버전 36개씩)
random.seed(42)
random.shuffle(loop3_data)

start_idx = (version - 1) * 36
end_idx = version * 36
survey_data = loop3_data[start_idx:end_idx]

print(f"Version {version}: 총 {len(survey_data)}개")

st.title('Loop 3 Counterfactual Example Selector')
...
filename = f'loop3_expert_response_{ts}.json'
st.download_button(
